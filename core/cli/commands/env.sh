...
env_help() {
	...
	echo
	log_info "Usage: omni env [options]"
	echo
	...
	echo
	printf "    ${D_CYAN}omni env set${NC}              # Interactive: add/update a variable\n"
	printf "    ${D_CYAN}omni env unset${NC}            # Interactive: select and remove a variable\n"
	printf "    ${D_CYAN}omni env ls${NC}               # List all export vars in rc file\n"
	echo
}
...
env_unset() {
	...
	if [[ ${#keys[@]} -eq 0 ]]; then
		...
		list_item "Add one first: ${D_CYAN}omni env set${D_NC}"
		separator
		return 0
	fi
...
env_ls() {
	...
	if [[ $count -eq 0 ]]; then
		...
		echo
		list_item "Add one: ${D_CYAN}omni env set${D_NC}"
	fi
