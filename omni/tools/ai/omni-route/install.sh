...
_omni_route_apply_platform_fixes() {
  ...
  local ig
  ig=$(find "$pkg_root/node_modules/omniroute/dist/node_modules/next" -path '*router-utils/instrumentation-globals.external.js' 2>/dev/null | head -1)
...
install_omni_route() {
  ...
  if _omni_route_ok; then
    ...
    _omni_route_apply_platform_fixes "$HOME/.omni/packages/omniroute"
    return 2
  fi
...
WRAPPER
  ...
  if command -v npm >/dev/null 2>&1 && npm i omniroute --prefix "$HOME/.omni/packages/omniroute" 2>>"$LOG_FILE"; then
    sed -i '1s|^#!/usr/bin/env node|#!/data/data/com.termux/files/usr/bin/node|' "$local_bin" 2>/dev/null
    for cmd in omniroute omni-route; do
      cat > "$PREFIX/bin/$cmd" <<'WRAPPER'
#!/data/data/com.termux/files/usr/bin/env bash
...
uninstall_omni_route() {
  ...
  rm -rf "$HOME/.omni/packages/omniroute"
...
update_omni_route() {
  ...
  if npm i omniroute@latest --prefix "$HOME/.omni/packages/omniroute" 2>>"$LOG_FILE"; then
    _omni_route_apply_platform_fixes "$HOME/.omni/packages/omniroute"
