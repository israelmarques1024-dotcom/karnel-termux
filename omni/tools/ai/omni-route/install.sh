...
case "$cmd" in
  list)
    (
      IFS=$'\n'
      for bin in $(command -v opencode claude codex qwen vibe mimo hermes kimi ollama odysseus openclaw freebuff pi agy mmx gentle-ai gga engram codegraph kilow command-code kimchi 2>/dev/null || true); do
        [ -n "$bin" ] && echo "$bin"
      done
    ) | sort
    ;;
...
uninstall_omni_route() {
  rm -f "$HOME/.local/bin/omni-route"
