...
`karnel voice` captura áudio do microfone, transcreve com o speech-to-text do Android,
...
```bash
karnel voice opencode          # Grava → edita → executa no opencode
karnel voice text              # Grava → edita → imprime no terminal
karnel voice "!"               # Atalho para "text"
karnel voice                   # Mostra ajuda
```
...
```bash
karnel voice opencode --lang pt-BR    # Português brasileiro
karnel voice claude-code --lang en-US # Inglês americano
karnel voice kimi-code --lang es      # Espanhol
```
...
```bash
karnel voice opencode --raw
karnel voice text --raw
```
...
|------|---------|---------|
| `kilo` | `kilo --prompt "..."` | `karnel voice kilo` |
| `opencode` | `opencode run "..."` | `karnel voice opencode` |
| `claude-code` | `claude -p "..."` | `karnel voice claude-code` |
| `codex` | `codex "..."` | `karnel voice codex` |
| `gemini-cli` | `gemini -p "..."` | `karnel voice gemini-cli` |
| `hermes-agent` | `hermes chat -q "..."` | `karnel voice hermes-agent` |
| `kimi-code` | `kimi -p "..."` | `karnel voice kimi-code` |
| `mimocode` | `mimo run "..."` | `karnel voice mimocode` |
| `mistral-vibe` | `vibe --prompt "..."` | `karnel voice mistral-vibe` |
| `openclaude` | `openclaude --bg "..."` | `karnel voice openclaude` |
| `pi` | `pi -p "..."` | `karnel voice pi` |
| `qwen-code` | `qwen -p "..."` | `karnel voice qwen-code` |
| `text` | stdout | `karnel voice text` |

## Fluxo Detalhado

```
1. karnel voice <agent>
         │
         ▼
...
karnel voice claude-code --lang en-US
...
karnel voice opencode --lang pt-BR
...
karnel voice text --raw --no-clip >> ideias.txt
```
...
```
karnel/cli/commands/voice.sh    → CLI command (dispatcher)
karnel/modules/voice.sh         → Module orchestrator (install/update/uninstall)
karnel/tools/voice/             → Tool installers
